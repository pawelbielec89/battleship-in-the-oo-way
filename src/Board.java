...
public class Board {
  ...
  */
  ...
  public boolean checkCoords(int x, int y) {
    ...
    if (this.getSquare(x, y).getCanShoot() == false) {
      dis.wrongCoordsMassage3();
    } else if (this.getSquare(x, y).getIsOccupiedArea()) {
      if (shipsAreCreated) {
        coordsAreProper = true;
      } else {
        dis.wrongCoordsMassage2();
      }
    } else {
      coordsAreProper = true;
    }
  ...
  public boolean checkIfCanSetShip(Ship ship) {
    ...
    for (int i = 0; i < ship.getLength(); i++) {
      try {
        if (this.getSquare(y, x).getIsOccupiedArea()) {
          return false;
        }
  ...
  public int[] coordinatesManager() {
    ...
    do {
      ...
      if (checkIfCoordsInBoard(coordinates)) {
        ...
        if (coordsAreProper != true) {
          coordinates = dis.chooseCoords();
        }
      } else {
