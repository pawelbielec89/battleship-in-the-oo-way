...
public class Display {
  ...
  public String chooseShip() {
    List<String> ships = new ArrayList<>(Arrays.asList("Destroyer", "Submarine", "Cruiser", "Battleship", "Carrier"));    
    int index;
    ...
    while (shipChosen == false) {
      ...
      if (index < 0 || index > 4) { print("There is no such an option!"); }  
      else { shipName = ships.get(index);
        if (usedShips.contains(shipName)) { print("You've already placed this ship!\n"); }
        else { usedShips.add(shipName); shipChosen = true; }  
      }
    }
    return shipName;
  }
  ...
  public boolean chooseIsVertical(){
    ...
    else if (position != 2) { print("There is no such an option!"); }
    }
    return isVertical;
  }
  ...
  public char assignCharInPosition(Board board, int x, int y) {
    ...
    if (sqr.getCanShoot()) {
      if (sqr.getIsShip() && board.getIsHidden() == false) { return 'X';
      } else if (sqr.getIsOccupiedArea() == true) { return '#'; }
      else { return '~'; }
    } 
    else { return 'O';}
  }
